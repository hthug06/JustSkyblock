...
public class LoadSchematic {
    ...
    public LoadSchematic(Location location,String worldName ,String fileName){
        ...
        ClipboardFormat format = ClipboardFormats.findByFile(new File(getServer().getPluginsFolder().getAbsoluteFile() + "/SkyblockPlugin/Schematic/" + fileName));

        if (!new File(getServer().getPluginsFolder().getAbsoluteFile() + "/SkyblockPlugin/Schematic/" + fileName).exists()){
